...
def card_value(ns):
    number = ns[0]
    suit = ns[1]
    return VALUE_TRANSLATION[number], suit
...
def hand_value(hand):
    ...
    if s_values == list(range(10, 15)) and n_suits == 1:
        rank = ROYAL_FLUSH
        highest_rank_value = 14
    elif n_suits == 1 and s_values[0] + 4 == s_values[4]:
        rank = STRAIGHT_FLUSH
        highest_rank_value = s_values[4]
    elif n_values == 2 and s_values[1] == s_values[3]:
        rank = FOUR_OF_A_KIND
        highest_rank_value = s_values[1]
        if s_values[0] == highest_rank_value:
            h1 = s_values[4]
        else:
            h1 = s_values[0]
    elif n_values == 2 and s_values[1] != s_values[3]:
        rank = FULL_HOUSE
        highest_rank_value = s_values[2]
        if s_values[0] == highest_rank_value:
            h1 = s_values[4]
        else:
            h1 = s_values[0]
        ...
        h4 = s_values[1]
        h5 = s_values[0]
    elif s_values[0] + 4 == s_values[4] and n_values == 5:
        rank = STRAIGHT
        highest_rank_value = s_values[4]
        h1 = s_values[4]
        h2 = s_values[3]
        ...
        h4 = s_values[1]
        h5 = s_values[0]
    elif n_values == 3 and s_values.count(s_values[2]) == 3:
        rank = THREE_OF_A_KIND
        highest_rank_value = s_values[2]
        h1 = max([x for x in s_values if x != highest_rank_value])
        h2 = min([x for x in s_values if x != highest_rank_value])
    elif n_values == 3 and s_values.count(s_values[1]) == 2:
        rank = TWO_PAIRS
        highest_rank_value = s_values[3]
        h1 = s_values[1]
        h2 = max([x for x in s_values if x != highest_rank_value and x != h1])
    elif n_values == 4:
        rank = ONE_PAIR
        highest_rank_value = max([x for x in s_values if s_values.count(x) == 2])
        other_values = [x for x in s_values if x != highest_rank_value]
        h1 = other_values[2]
        h2 = other_values[1]
        h3 = other_values[0]
    else:
    ...
    return rank, highest_rank_value, h1, h2, h3, h4, h5


def player1_wins(p1_hand, p2_hand):
    v_p1 = hand_value([card_value(i) for i in p1_hand])
    v_p2 = hand_value([card_value(i) for i in p2_hand])

    for i in range(7):
        if v_p1[i] != v_p2[i]:
            return v_p1[i] > v_p2[i]
    print("TIE!")
    return False


if __name__ == '__main__':
    rounds = [[x.split(" ")[:5], x.split(" ")[5:]] for x in hands.split("\n")]

    print(sum(player1_wins(e[0], e[1]) for e in rounds))
